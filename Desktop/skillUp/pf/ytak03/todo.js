...
//$('.task').on('click',function(e) {　　ではなく、下記の書き方
  $(document).on('click', '.task',function(e) {
    ...
    $('#hmemo').attr('placeholder',array[5]);
...
// 関数：追加処理
  function addition() {
    ...
    var mm = $('#hmemo').val();
    //エラーチェック
    var rtncd = errChk(no,dt,nm,sts,mm);
    ...
    switch (rtncd) {
      ...
      case 0:      //エラーがなければ、書込処理へ
        setStorage(no,dt,nm,sts,mm);   // ローカルストレージへの書込み
...
// 関数：入力時のエラーチェック
  function errChk(no,dt,nm,sts,mm){
    var rslt = 0;
    $('#hno').css('background-color','white');
    $('#hdate').css('background-color','white');
    $('#hname').css('background-color','white');
...
// 関数：ローカルストレージへの格納
  function setStorage(no,dt,nm,sts,mm){
...
// 関数：ローカルストレージからのデータの取得と表示
  function viewStorage() {
    ...
    for (var i=0; i < lists.length; i++) {
      ...
      str += '<td class="bdr">' + line.num + '</td>'
      str += '<td class="bdr">' + line.date + '</td>'
      str += '<td class="bdr">' + line.name + '</td>'
      str += '<td class="bdr">' + status + '</td>'
